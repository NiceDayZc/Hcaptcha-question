from components.ChatGPT import chat
from components.proxies import fetch_proxies
...
class hCaptcha(Session):
    def __init__(self, sitekey, host, proxies=False) -> None:
        super().__init__(
    ...
    def build_request(self, method, url, **kwargs):
        while True:
            try:
                return self.execute_request(method=method, url=url, **kwargs) 
    ...
    def fetch_captcha(self) -> dict:
        site_config = self.build_request(
            method='POST',
            url="https://hcaptcha.com/checksiteconfig", 
            params=(self.config | {'sc': '1','swa': '1'})
        ).json()
        #print(site_config)
        captcha = self.build_request(
            method='POST',
            url="https://hcaptcha.com/getcaptcha/%s" % (self.config['sitekey']), 
            data = (self.config | {'hl': 'en','motionData': self.motion_data(),'n': self.hsw(site_config['c']['req']),'c' : dumps(site_config['c'])})
        ).json()
        #print(captcha)
        return self.build_request(
            ...
            data = (self.config | {
                ...
                'old_ekey'  : captcha['key'],
                'extraData' : captcha,
                'motionData': self.motion_data(), 
                'n'         : self.hsw(captcha['c']['req']),        
                'c'         : dumps(captcha['c'])})
            ).json()
    ...
    def hsw(self, request: str) -> str:
        return check_output(["node", './components/hsw.js', request]).decode('utf-8').strip()
    
    def solve_gate(self, dict, task): 
        question = task["datapoint_text"]["en"]
        print(question)
        GPT = chat()
        result = GPT.conversation_result(
            question='%s answer only yes or no' % (question)
        )
        #print(result)
        if ('yes' in result.lower()):
            dict[task['task_key']] = {'text': 'yes'}
            
        else:
            dict[task['task_key']] = {'text': 'no'}

    @staticmethod
    def solve(sitekey, host, proxies=False):
        try:
            captcha = hCaptcha(sitekey, host, proxies=proxies)
            data = captcha.fetch_captcha()
            
            answers = {}
            threads = []

            for task in data['tasklist']:
                thread = Thread(target=captcha.solve_gate, args=(answers, task,))
            ...
            check_captcha = captcha.build_request(
                method='POST',
                url="https://hcaptcha.com:443/checkcaptcha/%s/%s" % (captcha.config['sitekey'], data['key']), 
                json={
                    'answers': answers,
                    'c': dumps(data['c']),
                    'job_mode': 'text_free_entry',
                    'motionData': captcha.motion_data(),
                    'n': captcha.hsw(data['c']['req']),
                    'serverdomain': captcha.config['host'],
                    'sitekey': captcha.config['sitekey'],
                    'v': captcha.config['v'],
            })
            
            if ('UUID' in check_captcha.text):
                return check_captcha.json()['generated_pass_UUID']
            else:
                return False
        except Exception as e:
            hCaptcha.solve(sitekey, host)